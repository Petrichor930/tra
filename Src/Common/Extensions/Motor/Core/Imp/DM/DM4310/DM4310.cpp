...
MotorTypeDef_e DM4310::checkBaseConfig()
{
    ...
    if (this->comType_ != ComType_e::FDCAN &&
        ...
        rslt |= 1;
        log.error(LOCATION, "DM4310", " %s: only support FDCAN or CAN comtype",
                  this->name_);
    }

    if (this->workMode_ == WorkMode_e::QUAD_VOLT) {
        rslt |= 1;
        log.error(LOCATION, "DM4310", " %s: WorkMode is not supported",
                  this->name_);
    }

    if (this->offsetId_ > 9) {
        rslt |= 1;
        log.error(LOCATION, "DM4310", " %s: Max Offset ID is only 9!",
                  this->name_);
    }

    if (this->txFreq_ > 1000) {
        rslt |= 1;
        log.error(LOCATION, "DM4310", " %s: Max TxFreq is only 1000!",
                  this->name_);
    }
