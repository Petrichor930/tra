...
template <typename Derived> class DMMotor : public MotorBase<DMMotor<Derived> > {
    // 定义一个模板类DMMotor，继承自MotorBase类，并使用Derived作为模板参数
    using Base = MotorBase<DMMotor<Derived> >;
...
public:
    ...
    void setTorq(float _torq) { cmd_.torq = _torq; }
    void setSpeed(float _speed) { cmd_.speed = _speed; }
    void setPos(float _pos) { cmd_.pos = _pos; }

    uint16_t canId() { return this->model_.txBaseId + this->offsetId_; }
    uint16_t masterId() { return this->model_.rxBaseId + this->offsetId_; }

    uint16_t uid() { return masterId(); } // TODO:

    // 定义两个公有成员函数，分别用于解析接收到的数据和控制电机
    void parse(uint8_t *_rxBuf)
    {
        // 先处理非常规数据反馈的帧
        if (_rxBuf[0] == static_cast<uint8_t>(canId()) &&
            ...
            this->data_.errorCode = fb.errorCode;
    ...
    bool ctrl()
    {
        ...
        case WorkMode_e::MIT_VDESPDES:
            ...
            DMMsg.msgMIT.Kd = float2uint(this->MITKd, -stats_.MITKpMax, stats_.MITKpMax, 12);
            DMMsg.msgMIT.Kp = float2uint(this->MITKp, -stats_.MITKpMax, stats_.MITKpMax, 12);
            DMMsg.msgMIT.torqueOffset = float2uint(cmd_.torq, -stats_.TMax, stats_.TMax, 12);
            isMIT = true;
            break;
    ...
    void registerReg(DMMotorReg_s *_regObj)
    {
        if (_regObj != nullptr) {
            regObjList_.insert({_regObj->regId, _regObj});
        }
    }
    void cancelReg(DMMotorRegId_e regId)
    {
        regObjList_.erase(regId);
    }
    bool writeReg(DMMotorRegId_e _regId, uint8_t dat[4])
    {
        bool rslt = true;
    ...
    bool readReg(DMMotorRegId_e _regId)
    {
        bool rslt = true;
    ...
    bool storageReg(DMMotorRegId_e _regId)
    {
        bool rslt = true;
