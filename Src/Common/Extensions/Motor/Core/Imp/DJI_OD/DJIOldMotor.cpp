                                 ...
                                 uint8_t _len)
{
    ...
    if (this->group_ == nullptr) {
        LOG::error("DJIOldMotor", " %s: Can't find group %hx", regInfo_.name,
                   this->getGroupId());
        return 1;
    } else {
        if (this->checkGroupSend(this->group_)) {
            // Check this Buffer
            // LOG::warn("DJIOldMotor", " %s: send data to CAN %hx", regInfo_.name,
            //           _sendId);
...
MotorTypeDef_e DJIOldMotor::parse(const RxBus_s::CANRxBuf_s<8> &_rxBuf)
{
    ...
    if (AUX_.rxFreq > 1.f)
        this->data_.spdRadps = angDiff * AUX_.rxFreq;
    ...
    case WorkMode_e::TRIP_VOLT: {
        if (this->cmd_.curCmdType == MotorCmdType_e::SET_ELEC) {
            ...
            if (this->torqPID_ != nullptr) {
                ...
                LOG::error("DJIOldMotor", " %s: torqPID is null",
                           regInfo_.name);
            }
